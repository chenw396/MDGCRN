...
def evaluate(model, mode):
    with torch.no_grad():
        ...
        for x, y in data_iter:
            ...
            loss = args.lamb * loss1 + args.lamb1 * loss2 + args.lamb2 * loss3
            ...
            mses.append(masked_rmse_loss(y_pred, y_true).item())
        ...
        mean_mae, mean_mape, mean_rmse = np.mean(maes), np.mean(mapes), np.mean(mses)#np.sqrt(np.mean(mses))
        l_3, m_3, r_3 = np.mean(l_3), np.mean(m_3), np.mean(r_3)
        l_6, m_6, r_6 = np.mean(l_6), np.mean(m_6), np.mean(r_6)
...
def traintest_model():  
    ...
    # lr_scheduler= torch.optim.lr_scheduler.ChainedScheduler([torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.steps, gamma=args.lr_decay_ratio),
    #                                                          torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[200], gamma=0.5)])
...
parser.add_argument('--max_diffusion_step', type=int, default=4, help='max diffusion step or Cheb K')
...
parser.add_argument("--steps", type=eval, default=[60, 110], help="steps")
parser.add_argument("--lr_decay_ratio", type=float, default=0.1, help="lr_decay_ratio")
parser.add_argument("--epsilon", type=float, default=8e-5, help="optimizer epsilon")
...
parser.add_argument("--cl_decay_steps", type=int, default=1000, help="cl_decay_steps")
...
shutil.copy2(f'model/{model_name}.py', path)
shutil.copy2('model/utils.py', path)
...
logger.info('separate loss lamb1', args.lamb1)
logger.info('compact loss lamb2', args.lamb2)
...
for category in ['train', 'val', 'test']:
    cat_data = np.load(os.path.join(f'{args.dataset}', category + '.npz'), allow_pickle=True)
