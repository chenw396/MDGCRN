...
class MDGCRN(nn.Module):
    ...
    def forward(self, x, y_cov, labels=None, batches_seen=None):
        ...
        g1 = F.softmax(F.tanh(torch.mm(node_embeddings1, node_embeddings2.T)), dim=-1)
        g2 = F.softmax(F.tanh(torch.mm(node_embeddings2, node_embeddings1.T)), dim=-1) #相似度矩阵
        supports = [g1, g2]
...
def main():
    ...
    parser.add_argument('--num_variable', type=int, default=275, help='number of variables (e.g., 207 in METR-LA, 325 in PEMS-BAY)')
    ...
    parser.add_argument('--rnn_units', type=int, default=32, help='number of hidden units')
